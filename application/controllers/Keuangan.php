...
class Keuangan extends MY_Controller
{
    ...
    public function create()
    {
        ...
        $date = explode('-', $data['input']->tanggal);
        if($data['input']->tipe == 'pemasukan'){
            $check_pengeluaran = $this->db->where("YEAR(tanggal) = ", $date[0])->where("MONTH(tanggal) = ", $date[1])
                                ->where('tipe', 'pengeluaran')->get('keuangan')->result();
            if(empty($check_pengeluaran)){
                $this->keuangan->insert([
                    'tanggal' => $data['input']->tanggal,
                    'jumlah_uang' => 0,
                    'tipe' => 'pengeluaran',
                    'keterangan' => 'default by system'
                ]);
            }
        } else {
            $check_pemasukan = $this->db->where("YEAR(tanggal) = ", $date[0])->where("MONTH(tanggal) = ", $date[1])
                                ->where('tipe', 'pemasukan')->get('keuangan')->result();
            if(empty($check_pemasukan)){
                $this->keuangan->insert([
                    'tanggal' => $data['input']->tanggal,
                    'jumlah_uang' => 0,
                    'tipe' => 'pemasukan',
